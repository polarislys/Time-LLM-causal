...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # trends: (B, N) - 每个batch中每个变量的趋势
        # 计算每个变量在时间维度上的平均变化率
        # x_enc: (B, T, N)
        # diff(dim=1): (B, T-1, N) - 时间维度上的差分
        # mean(dim=1): (B, N) - 对时间维度求平均，保留batch和变量维度
        trends_per_var = x_enc.diff(dim=1).mean(dim=1)  # (B, T-1, N) -> (B, N)
        ...
        # 获取因果知识提示（自适应）- 只生成 B 个
        causal_prompt_list = [""] * B
        if self.use_causal and hasattr(self, 'causal_module'):
            causal_prompt_list = self.causal_module.get_adaptive_causal_prompt(trends_per_var)

        # 只构造 B 个 prompt（每个 batch 一个）
        prompt = []
        for b in range(B):
            # 取第一个变量的统计量作为代表（或者可以取平均）
            min_values_str = str(min_values[b * N].tolist()[0])
            max_values_str = str(max_values[b * N].tolist()[0])
            median_values_str = str(medians[b * N].tolist()[0])
            lags_values_str = str(lags[b * N].tolist())
            ...
            prompt_ = (
                ...
                f"the trend of input is {'upward' if trends[b * N] > 0 else 'downward'}, "
        ...
        prompt_embeddings_B = self.llm_model.get_input_embeddings()(prompt_tokens.to(x_enc.device))  # (B, Lp, d_llm)

        # 扩展到 B*N（每个变量复用同一个 batch 的 prompt）
        Lp = prompt_embeddings_B.shape[1]
        prompt_embeddings = prompt_embeddings_B.unsqueeze(1).expand(B, N, Lp, self.d_llm).reshape(B * N, Lp, self.d_llm)
        ...
        # 整合因果soft tokens
        if self.use_causal and hasattr(self, 'causal_module'):
            causal_tokens = self.causal_module.forward(B * N).to(x_enc.device)
