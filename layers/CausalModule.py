...
class CausalModule(nn.Module):
    ...
    def compute_consistency_loss(self, input_data: torch.Tensor, predictions: torch.Tensor) -> torch.Tensor:
            ...
            计算因果一致性损失
            Args:
                input_data: 输入数据 (batch, seq_len, n_vars)
                predictions: 预测值 (batch, pred_len, n_vars)
            Returns:
                consistency_loss: 因果一致性损失
            """
            ...
            # 计算预测的时间差分作为趋势信号
            pred_trends = predictions[:, 1:, :] - predictions[:, :-1, :]  # (batch, pred_len-1, n_vars)
            
            loss = torch.tensor(0.0, device=predictions.device)
            count = 0
            
            # 对每个时间步计算因果一致性
            for t in range(min(pred_trends.shape[1] - 1, 10)):  # 限制最多检查10个时间步
                for cause_idx in range(n_vars):
                    for effect_idx in range(n_vars):
                        # 检查各个时间滞后的因果关系
                        for lag in range(1, min(t + 1, self.tau_max + 1)):
                            strength = causal_matrix[0, cause_idx, effect_idx, lag]
                            if abs(strength) > 0.1:  # 有显著因果关系
                                if t >= lag:
                                    cause_trend = pred_trends[:, t - lag, cause_idx]
                                    effect_trend = pred_trends[:, t, effect_idx]
                                    
                                    # 如果因果关系是正向的，cause和effect趋势应该相同
                                    # 如果是负向的，它们应该相反
                                    expected = torch.sign(cause_trend) * torch.sign(torch.tensor(strength, device=predictions.device))
                                    actual = torch.sign(effect_trend)
                                    
                                    # 计算因果一致性损失（符号不一致则惩罚）
                                    loss += torch.mean(torch.abs(expected - actual))
                                    count += 1
