...
class CausalModule(nn.Module):
    ...
    def get_adaptive_causal_prompt(self, trends: torch.Tensor) -> List[str]:
        ...
        for b in range(batch_size):
            # 找出最显著的趋势变量
            var_trends = [(i, abs(trends_np[b, i]), trends_np[b, i] > 0) 
                        for i in range(len(self.var_names))]
            var_trends.sort(key=lambda x: x[1], reverse=True)  # 按趋势大小排序
            
            explanation_chains = []
            
            # 对每个有显著趋势的变量，构建因果链
            for var_idx, trend_mag, is_upward in var_trends[:3]:  # 只关注前3个最显著的趋势
                if trend_mag < 0.01:  # 忽略小趋势
                    continue
                    
                var_name = self.var_names[var_idx]
                trend_dir = "increases" if is_upward else "decreases"
                
                # 1. 这个变量作为原因会影响什么
                effects = []
                for rel in self.causal_relationships:
                    if rel['cause'] == var_name and abs(rel['strength']) > 0.1:
                        effect_dir = "increase" if rel['strength'] * (1 if is_upward else -1) > 0 else "decrease"
                        effects.append(f"{rel['effect']} will {effect_dir} (lag={rel['lag']})")
                
                # 2. 哪些变量会影响这个变量
                causes = []
                for rel in self.causal_relationships:
                    if rel['effect'] == var_name and abs(rel['strength']) > 0.1:
                        cause_dir = "increase" if rel['strength'] * (1 if is_upward else -1) > 0 else "decrease"
                        causes.append(f"{rel['cause']} {cause_dir}s (lag={rel['lag']})")
                
                # 构建解释链
                chain = f"{var_name} {trend_dir}"
                if causes:
                    chain += f" because: {'; '.join(causes[:3])}"
                if effects:
                    chain += f". This causes: {'; '.join(effects[:3])}"
                
                explanation_chains.append(chain)
            
            # 组合提示
            if explanation_chains:
                prompt = "Causal Analysis: " + " | ".join(explanation_chains)
            else:
    ...
    def compute_consistency_loss(self, input_data: torch.Tensor, predictions: torch.Tensor) -> torch.Tensor:
        if not hasattr(self, 'causal_matrix') or self.causal_matrix is None:
            return torch.tensor(0.0, device=predictions.device)
        
        batch_size, pred_len, n_vars = predictions.shape
        causal_matrix = self.causal_matrix.to(predictions.device)
        pred_trends = predictions[:, 1:, :] - predictions[:, :-1, :]  # (B, pred_len-1, N)
        
        # 预先提取显著因果边（只做一次）
        if not hasattr(self, '_causal_edges'):
            edges = []
            for cause_idx in range(n_vars):
                for effect_idx in range(n_vars):
                    for lag in range(1, self.tau_max + 1):
                        strength = causal_matrix[0, cause_idx, effect_idx, lag].item()
                        if abs(strength) > 0.1:
                            edges.append((cause_idx, effect_idx, lag, strength))
            self._causal_edges = edges
        
        if len(self._causal_edges) == 0:
            return torch.tensor(0.0, device=predictions.device)
        
        # 向量化计算
        loss = 0.0
        count = 0
        max_t = min(pred_trends.shape[1] - 1, 10)
        
        for cause_idx, effect_idx, lag, strength in self._causal_edges:
            for t in range(lag, max_t + 1):
                if t >= lag and t < pred_trends.shape[1]:
                    cause_trend = pred_trends[:, t - lag, cause_idx]  # (B,)
                    effect_trend = pred_trends[:, t, effect_idx]  # (B,)
                    
                    # 向量化符号一致性检查
                    expected_sign = torch.sign(cause_trend) * torch.sign(torch.tensor(strength, device=predictions.device))
                    actual_sign = torch.sign(effect_trend)
                    loss += torch.mean(torch.abs(expected_sign - actual_sign))
                    count += 1
